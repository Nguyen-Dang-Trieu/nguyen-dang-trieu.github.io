...
Hình ảnh
Trong Linux, ngăn xếp IO được chia thành ba cấp độ chính, như minh họa trong hình:
...
- Lớp này quản lý hàng đợi IO (IO Queue) của các thiết bị khối (Block Devices) như ổ cứng.
- Nhiệm vụ chính là hợp nhất và sắp xếp các yêu cầu IO theo một thứ tự tối ưu, nhằm cải thiện hiệu suất truy cập đĩa.
...
3. Trả dữ liệu về không gian người dùng
...
- Blocking: Điều đó có nghĩa là sau khi gọi một hàm, người gọi đã đợi giá trị trả về của hàm và luồng ở trạng thái treo.
- Non-blocking: Điều đó có nghĩa là sau khi gọi một hàm, người gọi không đợi giá trị trả về của hàm và luồng tiếp tục chạy các chương trình khác (thực hiện các thao tác khác hoặc tiếp tục duyệt xem hàm có trả về giá trị hay không)
